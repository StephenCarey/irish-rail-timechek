...
get_station_code_exeption_test_cases = [("Connnly", "Station Connnly not found")]
...
next_train_test_cases = [
    (
        ...
        cork_departure_list,
        cobh_departure_list,
        "The next train to Cobh is 09:00",
    ),
    (
        "Cork",
        "Midleton",
        "MDLTN",
        cork_departure_list,
        midleton_departure_list,
        "The next train to Midleton is 09:15",
    ),
    (
        "Cork",
        "Belfast",
        "BFSTC",
        cork_departure_list,
        belfast_departure_list,
        "No train to Belfast due in the next period",
    ),
    ("Cork", "coob", "", "", "", "Station coob not found"),
    ("Cork", "Cobh", "COBH", "", "", "No station information available"),
]

station_info_test_cases = [("Cork", cork_departure_list, "P289")]
...
@pytest.mark.parametrize(
    "depart_station, arrive_station, station_code, departure_data, destination_data, expected",
    next_train_test_cases,
)
def test_next_train_to(
    depart_station,
    arrive_station,
    station_code,
    departure_data,
    ...
    requests_mock.get(
        "/realtime/realtime.asmx/getStationDataByCodeXML?StationCode=" + depart_station,
        text=departure_data,
    )
    requests_mock.get(
        "/realtime/realtime.asmx/getStationDataByCodeXML?StationCode=" + station_code,
        text=destination_data,
    )
...
def test_get_station_info(requests_mock):
    requests_mock.get(
        "/realtime/realtime.asmx/getStationDataByCodeXML?StationCode=Cork",
        text=cork_departure_list,
    )
